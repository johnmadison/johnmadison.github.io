...
var velocityIndex = 0;
var deathSpeed = 0.1;
var max_car_health = box2dfps * health;
...
function cw_createRandomCar() {
  ...
  car_def.wheelCount = rand(0,maxwheelCount); 
  
  car_def.wheel_radius = [];
  car_def.wheel_density = [];
  car_def.wheel_vertex = [];
  for (var i = 0; i < car_def.wheelCount; i++){
    car_def.wheel_radius[i] = rand(wheelMinRadius,wheelMaxRadius);
    car_def.wheel_density[i] = rand(wheelMinDensity,wheelMaxDensity);
  }
  
  car_def.chassis_density = rand(chassisMinDensity,chassisMaxDensity);

  car_def.vertex_list = new Array();
  car_def.vertex_list.push(new b2Vec2(rand(chassisMinAxis,chassisMaxAxis),0));
